...
async function checkAndUpdateInactiveUsers() {
  ...
  // We can remove the activeUsers array check since we're using the database
}
...
async function getInactiveUsers() {
  try {
    const inactiveUsers = await inactiveDB.find({}).lean();
    console.log("Fetched inactive users:", inactiveUsers);
    return inactiveUsers;  // This will be an array
  } catch (error) {
  ...
  client.on("messageCreate", async (message) => {
    if (!message.author.bot) {
      ...
      if (!blacklistedUser) {
        ...
        // Remove from inactive database if they were there
        await removeFromInactivityDB(message.author.id);
      }
    }
  });
